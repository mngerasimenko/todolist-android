...
Приложение подключается к REST API серверу `http://185.244.172.45:8090/` (только API, без веб-интерфейса).
...
- Фильтрация: Все / Активные / Выполненные (со счётчиками, по умолчанию «Активные»)
- Добавление новых задач (публичных и приватных) по кнопке «+» или клавишей отправки на клавиатуре
...
- Проверка совместимости версии клиента с сервером
- Неблокирующий диалог при недоступности сервера (Повторить / Продолжить)
...
- Snackbar-уведомления при ошибках и успешных действиях (создание/удаление задач, создание/вступление в список)
- Плавные анимации появления, удаления и перестановки задач в списке
...
```
Запуск приложения
  -> SplashViewModel проверяет авторизацию, наличие списка и статус сервера
     -> Сервер недоступен -> диалог (Повторить / Продолжить)
     -> Версия устарела -> блокирующий диалог (Обновить)
...
```
Пользователь вводит название задачи и нажимает "+" или клавишу отправки на клавиатуре
...
│   │   ├── ServerStatus.kt           # Статус сервера (Ok / Unavailable / UpdateRequired)
...
│       ├── StatusRepository.kt       # Интерфейс проверки статуса
...
│   │   │   ├── StatusApiService.kt   # Retrofit: статус сервера (публичный)
...
│   │   │   ├── StatusDto.kt          # StatusResponse (version, min_android_version)
...
│       ├── StatusRepositoryImpl.kt   # Реализация проверки статуса
...
| Метод  | Эндпоинт            | Описание                                    |
|--------|----------------------|---------------------------------------------|
| GET    | /api/status          | Статус, версия, min_android_version (публичный) |
